...
void LSM9DS0::initGyro() {
	...
	// TODO: I think it's rather an active high interrupt configuration.
...
uint8_t LSM9DS0::I2CreadByte(uint8_t address, uint8_t subAddress) {
	...
	TWIStart(); // ST
	TWIWrite(address); // SAD + W
	TWIWrite(subAddress); // SUB
	TWIStart(); // SR
	TWIWrite(address | 1); // SAD + R
